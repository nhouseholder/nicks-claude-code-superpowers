...
- **Sports prediction algorithms**: UFC (mmalogic), MLB+NHL (diamondpredictions), NBA+NCAA (courtside-ai), NHL (icebreaker-ai) — Python, walk-forward backtesting, Firestore, Cloudflare
...
All projects live in iCloud Drive under `~/Projects/` — flat structure, no categories. **`~/Projects/` is a symlink or alias to `~/Library/Mobile Documents/com~apple~CloudDocs/Projects/`.**
...
├── icebreaker-ai/         (icebreaker-ai.pages.dev — NHL predictions)
...
- `~/Projects/` lives on iCloud Drive. The full path is `~/Library/Mobile Documents/com~apple~CloudDocs/Projects/`.
...
| icebreaker-ai.pages.dev | icebreaker-ai | icebreaker-ai | ~/Projects/icebreaker-ai |
