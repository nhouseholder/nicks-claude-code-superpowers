---
name: full-handoff
description: Generate a comprehensive handoff document, sync to all locations, and archive old ones. This is the LAST thing you do before ending a session.
---
...
### 3b. Fresh-Session Guard (MANDATORY — prevents overwriting substantive handoffs)

If this is a brand-new session with NO code changes (no commits since the previous handoff, no uncommitted changes, no tasks completed beyond the handoff itself), then the previous handoff contains all the important context. Do NOT overwrite it with a thin "handoff-only" document.

**Detection logic:**
```bash
# Check: any commits since the previous handoff was written?
PREV_HANDOFF_DATE=$(head -1 "$PROJECT_PATH/HANDOFF.md" 2>/dev/null | grep -oE '[0-9]{4}-[0-9]{2}-[0-9]{2}')
COMMITS_SINCE=$(git log --oneline --since="$PREV_HANDOFF_DATE" 2>/dev/null | wc -l | tr -d ' ')
UNCOMMITTED=$(git status --short 2>/dev/null | wc -l | tr -d ' ')
echo "Commits since last handoff: $COMMITS_SINCE | Uncommitted: $UNCOMMITTED"
```

**If BOTH are 0 (no new commits AND no uncommitted changes):**
1. **DO NOT overwrite HANDOFF.md** — the existing one is the substantive handoff
2. Instead, READ the previous handoff and present a summary to the user
3. Say: "No new work since the last handoff. The existing handoff is still current: [filename]. Here's what it covers: [summary of sections 1, 2, 6, 8]"
4. **SKIP Steps 4-9** — no new handoff needed
5. If the user still wants a new handoff, incorporate ALL content from the previous handoff into the new one:
   - Copy sections 1-10 from the previous handoff as a "Previous Session" block
   - Add a new "Current Session" note: "No changes — handoff refreshed on [date]"
   - Preserve all next steps, in-progress items, and blocked items verbatim

**If there ARE new commits or uncommitted changes:** proceed to Step 4 normally.
