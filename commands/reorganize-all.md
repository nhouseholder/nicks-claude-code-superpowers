...
mv ~/Projects/all-things-ai ~/Projects/apps/all-things-ai

# windsurf-skills-only — move to tools/
mv ~/Projects/windsurf-skills-only ~/Projects/tools/windsurf-skills-only
...
[ -d ~/Projects/sports/loss-analyst ] && [ -d ~/Projects/loss-analyst ] && \
...
After reorganization, update `~/.claude/memory/topics/ufc_canonical_paths.md` and all references to use `~/Projects/sports/ufc-predict/` as the canonical path.
