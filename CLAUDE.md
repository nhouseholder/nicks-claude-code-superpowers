...
3. **Actual odds must be sourced, never assumed. Missing odds = RUN THE SCRAPER.** Scrape or look up the real odds for every bet. If odds are missing, DO NOT accept it — run the odds scraper/backfill script (BestFightOdds, DraftKings, etc.) to fetch them. "—" in a payout cell is NEVER acceptable. Only after the scraper confirms the source is genuinely unavailable (page deleted, event too old) can you note "odds unavailable — scraper checked [source] on [date]". NEVER display "—" and call it correct.
...
**CRITICAL — What "stale" means (THIS HAS CAUSED CATASTROPHIC DATA LOSS):**
- **STALE** = a file that has been EXPLICITLY REPLACED by a newer version (v2 replaced by v3, old backtester replaced by new one, user says "this is outdated")
- **NOT STALE** = a file that differs from git, another copy, or production. Difference ≠ staleness. The different version may be NEWER (deployed but uncommitted, work in progress, production-only changes).
- **NEVER classify files as stale based on absence from git or difference from another system.** Staleness requires EVIDENCE of replacement: a newer file doing the same job, a commit saying "replaced X with Y", or explicit user instruction.
- **When in doubt, a file is NOT stale.** Ask the user before archiving anything without clear replacement evidence.
...
All projects live in iCloud Drive under `~/Projects/` — flat structure, no categories. **`~/Projects/` is a symlink to `~/Library/Mobile Documents/com~apple~CloudDocs/ProjectsHQ/`.**

```
~/Projects/
├── mmalogic/              (mmalogic.com — UFC predictions)
├── diamondpredictions/    (diamondpredictions.com — MLB+NHL predictions)
├── courtside-ai/          (courtside-ai.pages.dev — NBA+NCAA predictions)
├── icebreaker-ai/         (ARCHIVED — NHL consolidated into diamondpredictions)
├── enhancedhealthai/      (enhancedhealthai.com — health tech)
├── mystrainai/            (mystrainai.com — cannabis recommendations)
├── nestwisehq/            (nestwisehq.com — family finance)
├── researcharia/          (researcharia.com — AI research)
├── all-things-ai/         (all-things-ai.pages.dev)
├── march-madness-2026/    (seasonal NCAA tournament)
├── nfl-draft-predictor/   (NFL draft analysis)
├── significant-bets/      (cross-sport bet tracker)
├── loss-analyst/          (AI loss analysis)
├── ophtho-cards/          (ophthalmology flashcards)
├── recipe-cards/          (recipe app)
├── recipes-app/           (recipe app v2)
├── claude-glm-router/     (model router proxy)
├── windsurf-skills-only/  (windsurf skills)
├── friends-bday/          (birthday tracker)
├── strain-tracker/        (strain logging)
├── site-to-repo-map.json
└── project-manifest.json
```

**Rules:**
- `~/Projects/<project>/` is the ONLY canonical location. Two levels only: Projects → project folder.
- `~/Projects/` lives on iCloud Drive. The full path is `~/Library/Mobile Documents/com~apple~CloudDocs/ProjectsHQ/`.
...
- `cannalchemy-v2` and `strain-finder-real` are ARCHIVED — use `mystrainai` for ALL cannabis work.
- `mlb-predict` is ARCHIVED — use `diamondpredictions` for ALL MLB+NHL work.
...
|-----------|-------------------|-------------|------------|
| mmalogic.com | octagonai | ufc-predict | ~/Projects/mmalogic |
| diamondpredictions.com | diamond-predict | diamond-predictions | ~/Projects/diamondpredictions |
| mystrainai.com | mystrainai | Strain-Finder-Front-Cannalchemy-Back | ~/Projects/mystrainai |
| enhancedhealthai.com | enhancedhealthai | enhanced-health-ai | ~/Projects/enhancedhealthai |
| nestwisehq.com | Workers | dad-financial-planner | ~/Projects/nestwisehq |
| researcharia.com | Workers | aria-research | ~/Projects/researcharia |
| courtside-ai.pages.dev | courtside-ai | courtside-ai | ~/Projects/courtside-ai |
| icebreaker-ai.pages.dev | ARCHIVED | icebreaker-ai | ARCHIVED — NHL now in diamondpredictions |
...
- **ALSO check the LIVE SITE version** — `curl -s <site-url> | grep version` or check via Claude in Chrome. If the live site has a HIGHER version than git, that means production has changes not in git. STOP and commit those changes before deploying.
...
- **Cloudflare deploys are IRREVERSIBLE.** Workers Sites purges old assets from KV on every deploy. There is NO rollback for asset deletion. If you deploy stale code, the good assets are permanently destroyed. This caused total loss of the researcharia.com redesign (2026-03-25).
...
### FAILSAFE 5: Commit Before Switch OR Deploy
Before switching to a different project, branch, or directory — AND before ANY deploy:
...
- **NEVER deploy uncommitted code.** `git add` + `git commit` + `git push` MUST happen BEFORE `wrangler deploy` or any equivalent. Deploying uncommitted code means the next deploy from git will REVERT your changes. This caused a catastrophic site reversion on researcharia.com (2026-03-25).
- **NEVER do ad-hoc deploys.** Use `/site-update` or `/deploy` commands which enforce commit-before-deploy. Raw `wrangler deploy` without the command pipeline is how reversions happen.
...
### FAILSAFE 9: Check Cloudflare Before Deploying (CATASTROPHIC INCIDENT — 2026-03-26)

**A `wrangler deploy` from git destroyed a frontend redesign that existed only on Cloudflare. The files were permanently lost — KV assets are purged on deploy and rollback breaks static serving.**

Before ANY `wrangler deploy` or `npm run deploy`:

1. **Check Cloudflare deployment history:**
   ```bash
   npx wrangler@3.99.0 deployments list 2>&1 | tail -20
   ```
2. **Compare last CF deploy date to last git commit touching public/:**
   ```bash
   git log -1 --format='%ci' -- public/
   ```
3. **If CF was deployed MORE RECENTLY than git's last public/ commit: STOP.**
   Someone deployed changes directly to CF without committing. Deploying from git will OVERWRITE those changes with NO recovery path.
4. **Visually verify** the live site matches your local `public/` files before deploying.
5. **ALWAYS commit all frontend changes to git BEFORE deploying.** A deploy without a git commit backing it is a ticking time bomb.
6. **The deploy-guard.py hook** (PreToolUse) enforces this automatically, but agents must also check manually.

**This failsafe is enforced by:** `~/.claude/hooks/deploy-guard.py` (PreToolUse hook on all Bash commands containing `wrangler deploy`).
...
19. **Never delete commands, skills, or hooks without user confirmation** — Before removing, overwriting, or replacing ANY file in `~/.claude/commands/`, `~/.claude/skills/`, or `~/.claude/hooks/`: (a) list what you're about to remove, (b) explain why, (c) get explicit user approval. This includes indirect deletion via `cp` that overwrites, `mv` that replaces, or `Write` that overwrites. Archive instead of delete. The user has invested significant time building these — losing them silently is unacceptable.
    **PROTECTED SKILLS (from aitmpl.com — NEVER modify, delete, or overwrite):**
    These 11 skills were installed from AI Templates and are READ-ONLY. They are pre-built professional agent protocols. Claude may READ their SKILL.md to follow the protocol, but must NEVER edit, replace, or "improve" them:
    `frontend-design`, `code-reviewer`, `senior-architect`, `senior-backend`, `senior-frontend`, `skill-creator`, `ui-design-system`, `ui-ux-pro-max`, `webapp-testing`, `senior-prompt-engineer`, `brainstorming`
    If a protected skill needs updating, tell the user to reinstall from aitmpl.com. Do NOT modify in place.
...
24. **NEVER deploy without committing first** — Before ANY `wrangler deploy`: (a) `git status` to check for uncommitted changes, (b) commit ALL changes to git, (c) run FAILSAFE 9 checks (compare CF deploy history to git history), (d) only then deploy. A deploy that isn't backed by a git commit means the next deploy from git will OVERWRITE those changes. This destroyed a frontend redesign on 2026-03-26. The deploy-guard.py hook enforces this, but agents must also check manually.
25. **NEVER deploy without visually verifying** — After any deploy, open the live site in a browser (Claude in Chrome) and verify it matches expectations. Don't trust `curl` grep alone — visual bugs are only visible in a browser.
27. **SURGICAL SCOPE — never touch what you weren't asked to touch** — When given a focused task (algorithm update, config change, data fix), ONLY modify the files directly related to that task. DO NOT: (a) "clean up" nearby HTML/CSS/frontend files, (b) refactor UI components that happen to be in the same directory, (c) "improve" page layouts, admin panels, or dashboards while passing through, (d) remove, rename, or restructure frontend files during a backend change, (e) change ANY file you weren't explicitly asked to change. If the task is "update the algorithm," you touch the algorithm files and NOTHING ELSE. If you notice frontend issues while doing backend work, NOTE them for the user — do NOT fix them. This rule exists because Claude has repeatedly destroyed admin pages, lost frontend designs, and broken working UIs by making unsolicited "improvements" during focused backend tasks. **The blast radius of a change must match the scope of the request.**
29. **NEVER disconnect working integrations** — Before editing ANY file that contains API calls, webhook URLs, GitHub Actions triggers (`workflow_dispatch`), OAuth configs, service worker registrations, cron triggers, or external service connections: (a) identify ALL integrations in the file, (b) preserve them EXACTLY as-is unless the user explicitly asked to change them, (c) after editing, verify every integration still works (test the endpoint, check the button, confirm the webhook). This includes: GitHub Actions buttons on admin pages, automated deploy triggers, daily cron jobs, "Generate Picks" buttons, "Refresh Data" buttons, and any UI element that calls an external API. Claude has repeatedly broken these by "simplifying" code that contained critical integration wiring. **If you don't understand what a piece of code does, DON'T TOUCH IT.**
30. **NEVER rename/move/delete directories with active sessions** — Claude Code sessions are tied to their absolute path. Renaming a parent directory (e.g., `***Projects***` → `ProjectsHQ`) kills ALL sessions opened from that path with NO recovery. Before ANY directory rename/move: (a) warn the user how many sessions will die, (b) verify handoffs are CURRENT (not stale from session start), (c) get explicit approval. This killed 7 sessions and lost hours of in-progress context on 2026-03-26.
32. **Max 2 subagents, Opus only, shared context** — NEVER spawn more than 2 subagents. NEVER spawn Sonnet/Haiku agents for tasks that require judgment (design, debugging, data analysis, parameter sweeps). All agents MUST be briefed with: (a) anti-patterns.md, (b) project memory, (c) the specific domain rules they need. Agents that write to files MUST write to SEPARATE output files — never to the same file (this caused 5 agents to clobber each other's registry results on 2026-03-26). For parameter sweeps: run SEQUENTIALLY, not in parallel — each run reads the previous result before starting. For audits: max 2 agents (frontend + backend), never 5. If you think you need 3+ agents, you're wrong — do it yourself sequentially.
31. **User correction = MANDATORY triple-write** — When the user corrects you (says something is wrong, points out an error, shows a bug you caused, expresses frustration at a mistake), you MUST write the lesson to ALL THREE of these locations before continuing:
   **(a) Project memory** — `~/.claude/projects/<project>/memory/` as a feedback or error memory file. This is the per-project persistent record.
   **(b) Anti-patterns** — `~/.claude/anti-patterns.md` with the standard format (Context, Bug, Root cause, Fix, Flawed assumption, Applies when). This is the cross-project error memory.
   **(c) GitHub superpowers repo** — sync via `/tmp/` clone to `nhouseholder/nicks-claude-code-superpowers`. This is the shared persistent memory that survives session death, machine changes, and context compaction.
   Additionally, if the correction is about a **site-specific** issue (UFC tables, diamond picks, courtside admin, etc.), ALSO update the relevant site command (`/mmalogic`, `/update-diamond`, etc.) with the learned rule.
   **Detection signals:** "no", "that's wrong", "I already told you", "stop", "why did you", "that was a win not a loss", "this keeps happening", frustration language, screenshots showing bugs you caused, user re-explaining something. When in doubt, record it — a false positive (recording something that didn't need recording) is harmless. A false negative (NOT recording a correction) means the next session makes the same mistake.
...
When you modify ANY file under `~/.claude/skills/`, `~/.claude/CLAUDE.md`, `~/.claude/hooks/`, `~/.claude/commands/`, or `~/.claude/anti-patterns.md`:

1. Clone the superpowers repo to `/tmp/superpowers-sync` (fresh clone, never from iCloud)
2. Copy the changed files into the clone
3. Commit with a clear message describing what changed
4. Push to GitHub (`nhouseholder/nicks-claude-code-superpowers`)
5. Clean up the `/tmp/` clone

This ensures all rules, skills, and hooks are version-controlled and recoverable. The superpowers repo at `~/Library/Mobile Documents/com~apple~CloudDocs/superpowers` is the iCloud copy — never git push from there. Always clone to `/tmp/` first.
